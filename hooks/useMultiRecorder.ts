...
export const useMultiRecorder = ({ userStream, displayStream, compositeStream }: UseMultiRecorderProps) => {
  ...
  const startRecording = useCallback(() => {
    ...
    const startStreamRecorder = (stream: MediaStream, chunksRef: React.MutableRefObject<Blob[]>) => {
       try {
    ...
    if (displayStream) {
      displayRecorderRef.current = startStreamRecorder(displayStream, displayChunksRef);
    }

    if (compositeStream) {
      compositeRecorderRef.current = startStreamRecorder(compositeStream, compositeChunksRef);
    }
  ...
  const stopRecording = useCallback(() => {
    ...
    const finalize = () => {
      ...
      const userBlob = userChunksRef.current.length > 0 
        ? new Blob(userChunksRef.current, { type }) 
        : undefined;
        
      const displayBlob = displayChunksRef.current.length > 0 
        ? new Blob(displayChunksRef.current, { type }) 
        : undefined;

      const compositeBlob = compositeChunksRef.current.length > 0
