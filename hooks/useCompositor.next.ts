...
export const useCompositor = ({
  ...
  useEffect(() => {
    ...
    return () => {
      if (userVid.srcObject)
        (userVid.srcObject as MediaStream).getTracks().forEach((t) => t.stop());
      if (displayVid.srcObject)
        (displayVid.srcObject as MediaStream)
          .getTracks()
  ...
  useEffect(() => {
    if (userVideoRef.current && userStream) {
      ...
      userVideoRef.current
        .play()
        .catch((e: Error) => console.log("Composite user play err", e));
    }
  }, [userStream]);
  ...
  useEffect(() => {
    if (displayVideoRef.current && displayStream) {
      ...
      displayVideoRef.current
        .play()
        .catch((e: Error) => console.log("Composite display play err", e));
    }
  }, [displayStream]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (
        displayVideoRef.current &&
        displayStream &&
        displayVideoRef.current.readyState >= 2
      ) {
      ...
      if (
        userVideoRef.current &&
        userStream &&
        userVideoRef.current.readyState >= 2
      ) {
